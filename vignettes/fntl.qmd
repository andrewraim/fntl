...
`double` - the expected result of a `fntl::dfv` - but something else that can
be readily converted to a `double`.
...
use it with R functions. This is accomplished by wrapping an `Rcpp::Function`
into a lambda. This will incur the usual overhead of calling R code from C++,
