...
Suppose $f(\vec{x},\vec{x}') : \mathbb{R}^d \times \mathbb{R}^d \rightarrow \mathbb{R}$ and
$\vec{X} \in \mathbb{R}^{n \times d}$ is a matrix with rows $\vec{x}_1, \ldots, \vec{x}_n$. Also let $\vec{a} = [a_1 \cdots a_n]^\top$ be a fixed vector. The `outer` operation
...
Specifically, let $\vec{X} \in \mathbb{S}^{m \times n}$ be a matrix whose elements
...
function and suppose $k$ of the $mn$ elements in $\vec{X}$ satisfy the indicator.
...
$$
<!-- -->
where each pair $(i_\ell, j_\ell)$ are coordinates of an element in $\vec{X}$ that
