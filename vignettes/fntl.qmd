---
title: "`fntl`: `{r} packageDescription('fntl', fields = 'Title')`"
author:
  - name: Andrew M. Raim
...
`as` and `wrap` mechanisms, respectively.
...
In the `fntl` source code, objects which support `as` and `wrap` are implemented in a particular way: structures are defined before `Rcpp.h` is included, then `Rcpp.h` is included, and finally serialization operations are defined. The `wrap` and `as` operations are defined via member functions (referred to as the "intrusive" implementation). Details on implementing these mechanisms are found in the [Rcpp Extending][rcpp-extend] vignette, a detailed post on the [Rcpp Gallery][gallery-seamless], and helpful threads on Stack Overflow [here][so-51110244] and [here][so-74887786] .

[rcpp-extend]: https://cran.r-project.org/web/packages/Rcpp/vignettes/Rcpp-extending.pdf
[gallery-seamless]: https://gallery.rcpp.org/articles/custom-templated-wrap-and-as-for-seamingless-interfaces/
[so-51110244]: https://stackoverflow.com/q/51110244
[so-74887786]: https://stackoverflow.com/q/74887786
...
To illustrate performance characteristics, let us consider a brief simulation of the relationship between mean-squared error (MSE) and sample size in a logistic regression, suppose $Y_i \sim \text{Ber}(p_i)$ are independent with $p_i = \text{logit}^{-1}(\beta_0 + \beta_1 x_i)$. Data-generating values of the coefficients are taken to be $\beta_0 = 0$ and $\beta_1 = 1$, respectively. We take $R$ draws of $(y_1, \ldots, y_n)$ and compute the MLE $\hat{\vec{\beta}}^{(r)} = (\hat{\vec{\beta}}_0^{(r)}, \hat{\vec{\beta}}_1^{(r)})$ for the $r$th draw using the L-BFGS-B optimization method. The MSE associated with sample size $n$ is computed as $\text{MSE}_n = \frac{1}{R} \sum_{r=1}^R \lVert \hat{\vec{\beta}}^{(r)} - \vec{\beta} \rVert^2$; this is computed for sample sizes $n \in \{ 100, 200, 500, 1000, 10000 \}$. We describe four versions of the code; to see the implementations, refer to the corresponding source files in the `examples` folder.
...
## Templates for Rcpp Vectors and Matrices {#sec-rpp-templates}

**TBD:** Under Construction

Rcpp matrix and vector types are based on templated classes that parameterize the types of underlying elements which they contain. For example, `NumericMatrix` is a typedef for `Matrix<REALSXP>` while `IntegerVector` is a typedef for `Vector<INTSXP>`. For others, see the [Rcpp API][rcpp-matrices-src]. We make use of the template classes in several places in the `fntl` API to support multiple Rcpp types with the same API function, especially in @sec-matrix. Rcpp vector and matrix arguments will appear as `Vector<RTYPE>` and `Matrix<RTYPE>` with a `RTYPE` template argument under these circumstances.

[rcpp-matrices-src]: https://dirk.eddelbuettel.com/code/rcpp/html/instantiation_8h.html

## Using RcppArmadillo

The `RcppArmadillo` package [@EddelbuettelSanderson2014] greatly facilitates
numerical matrix operations in C++. While the `RcppArmadillo` API is not used
in the `fntl` interface, `fntl` users may want to make use of the
`RcppArmadillo` in their code. To use `RcppArmadillo` in code with `fntl`,
ensure that `RcppArmadillo.h` is included before `fntl.h`.

Consider an example of maximizing the likelihood

$$
L(\sigma^2) \propto (\sigma^2)^{-n/2}
\exp\left\{ -\frac{1}{2 \sigma^2} (\vec{y} - \vec{\mu})^\top (\vec{y} - \vec{\mu}) \right\},
$$

where $\vec{y} = (y_1, \ldots, y_n)$ and $\vec{\mu} = (\mu_1, \ldots, \mu_n)$ are fixed.
The following example carries this out by maximizing $\log L(\sigma^2)$ using
Brent's algorithm.

```{.cpp include="examples/arma.cpp"}
```

If we change the code to include `fntl.h` before `RcppArmadillo.h`, the
following error is produced by the compiler.

```
#error "The file 'Rcpp.h' should not be included. Please correct to include
only 'RcppArmadillo.h'."
```

The issue is resolved by including `RcppArmadillo.h` before `fntl.h`, as shown
in the display. We may compile and invoke this function as usual.
...
Rcpp::sourceCpp("examples/arma.cpp")
arma_example(y, mu)
```
...
## Optional Arguments {.unnumbered .unlisted}
...
## Status Codes {.unnumbered .unlisted}
...
<!--
For some reason, references `@sec-fd-deriv` have not been working. A workaround
is to instead write `[@sec-fd-deriv]`. It's not clear to me what the issue is
or why only this tag seems to be affected ...
-->

Compute the first and second derivatives of
$f : \mathbb{R}^n \rightarrow \mathbb{R}$ numerically at point $\vec{x}$.
Denote $\vec{e}_i$ as the $i$th column of an $n \times n$ identity matrix.
First derivatives in the $i$th coordinate are computed as
<!-- -->
\begin{align*}
&\frac{\partial f(\vec{x})}{\partial x_i} \approx \frac{
f(\vec{x} + h \vec{e}_i) - f(\vec{x} - h \vec{e}_i)
}{2h}, \\
%
&\frac{\partial f(\vec{x})}{\partial x_i} \approx 
\frac{ f(\vec{x} + h \vec{e}_i) - f(\vec{x}) }{h}, \\
%
&\frac{\partial f(\vec{x})}{\partial x_i} \approx 
\frac{ f(\vec{x}) - f(\vec{x} - h \vec{e}_i) }{h},
\end{align*}
<!-- -->
...
<!-- -->
\begin{align*}
&\frac{\partial^2 f(\vec{x})}{\partial x_i x_j} \approx
\frac{
f(\vec{x} + h_i \vec{e}_i + h_j \vec{e}_j) - f(\vec{x} + h_i \vec{e}_i - h_j \vec{e}_j) - f(\vec{x} - h_i \vec{e}_i + h_j \vec{e}_j) +
f(\vec{x} - h_i \vec{e}_i - h_j \vec{e}_j)
}{4 h_i h_j}, \\
%
&\frac{\partial^2 f(\vec{x})}{\partial \vec{x}_i \vec{x}_j} \approx
\frac{
f(\vec{x} + h_i \vec{e}_i + 2 h_j \vec{e}_j) - f(\vec{x} + h \vec{e}_i) - f(\vec{x} + h_j \vec{e}_j) + f(\vec{x})
}{ h_i h_j}, \\
%
&\frac{\partial^2 f(\vec{x})}{\partial x_i x_j} \approx
\frac{
f(\vec{x}) - f(\vec{x} - h_i \vec{e}_i) - f(\vec{x} - h_j \vec{e}_j) + f(\vec{x} - h_i \vec{e}_i - h_j \vec{e}_j)
}{h_i h_j},
\end{align*}
<!-- -->
...
$\vec{x}$ and the choice of $h$. See Section 5.7 of @PressEtAl2007 for discussion.
...
```
1. Function $f$ to differentiate.
2. Point $\vec{x}$ at which derivative is taken.
...
Compute the first and second derivatives of $f(\vec{x}) = \sin(\vec{b}^\top \vec{x})$ at $\vec{x}_0 = (1/2, \ldots, 1/2)$ where $\vec{b} = (1, \ldots, n)$. The gradient and Hessian are given in closed-form by
<!-- -->
\begin{align*}
&\frac{\partial f(\vec{x})}{\partial \vec{x}} = \vec{b} \cos(\vec{b}^\top \vec{x}), \\
&\frac{\partial^2 f(\vec{x})}{\partial \vec{x} \partial \vec{x}^\top} = -\vec{b} \vec{b}^\top \sin(\vec{b}^\top \vec{x}).
\end{align*}
<!-- -->
...
Compute the first and second derivatives of
$f : \mathbb{R}^n \rightarrow \mathbb{R}$ numerically at point $\vec{x}$ using
...
(symmetric, forward, or backward), $h > 0$ to be an initial step size, and
$\delta \in (0,1)$ to be a reduction factor, the method computes a table of
values
<!-- -->
\begin{align*}
A_{mq} = 
...
Taking $n = 0$ produces finite differences described in [@sec-fd-deriv] using
the initial step size $h$ as the perturbation. Here the achieved tolerance is
...
This is a continuation of the example in [@sec-fd-deriv]. Compute first and second derivatives of $f(\vec{x}) = \sin(\vec{b}^\top \vec{x})$ at $\vec{x}_0 = (1/2, \ldots, 1/2)$ with $\vec{b} = (1, \ldots, n)$. Let us again define the point $\vec{x}_0$.
...
```
1. Function whose gradient is to be computed.
...
Compute the gradient of $f(\vec{x}) = \vec{x}^\top \vec{x}$. A C++ function with Rcpp interface is defined in the file `examples/gradient.cpp`.
...
\vdots & \ddots & \vdots \\
\frac{\partial f_m(\vec{x})}{\partial x_1} & \cdots & \frac{\partial f_m(\vec{x})}{\partial x_n}
\end{bmatrix}.
$$
<!-- -->
Each coordinate is computed via `deriv` in @sec-deriv.
...
```
1. Function whose Jacobian is to be computed.
...
Compute the Jacobian of $f(\vec{x}) = [f_1(\vec{x}), \ldots, f_5(\vec{x})]$, $f_i(\vec{x}) = \sum_{j=1}^i \sin(x_j)$, at the point $\vec{x} = (1,2,3,4,5)$. To obtain the resulting value as an $m \times n$ matrix, we apply the `Rcpp::List` conversion operator to the result. A C++ function with Rcpp interface is defined in the file `examples/jacobian.cpp`.
...
<!-- -->
$$
\frac{\partial^2 f(\vec{x})}{\partial \vec{x} \partial \vec{x}^\top} =
\begin{bmatrix}
\frac{\partial^2 f(\vec{x})}{\partial x_1 \partial x_1} &
\cdots &
\frac{\partial^2 f(\vec{x})}{\partial x_1 \partial x_n} \\
\vdots & \ddots & \vdots \\
\frac{\partial^2 f(\vec{x})}{\partial x_n \partial x_1} &
\cdots &
\frac{\partial^2 f(\vec{x})}{\partial x_n \partial x_n}
\end{bmatrix}.
$$
<!-- -->
...
2. Point $\vec{x}$ at which Hessian is taken.
3. Optional arguments.
4. Type of finite difference to use. See the definition of `fd_types` in
   [@sec-fd-deriv].
...
Compute the Hessian of $f(\vec{x}) = \sum_{i=1}^n \sin(x_i)$ at $\vec{x} = (1, 2)$. To
obtain the resulting value as an $n \times n$ matrix, we apply the `Rcpp::List` conversion operator to the result. A C++ function with Rcpp interface is defined in the file `examples/hessian.cpp`.
...
```
1. Scaling factor to be applied to the value of $f(\vec{x})$ during optimization. Use
...
This section presents methods to minimize a function $f : \mathbb{R}^n \rightarrow \mathbb{R}$ from a given starting value $\vec{x}_0$.
...
Minimize the function $f(\vec{x}) = \sum_{i=1}^n x_i^2 - 1$ for $n = 2$. Take $\vec{x}_0 = (1,-1)$ as the initial value. A C++ function with Rcpp interface is defined in the file `examples/neldermead.cpp`.
...
Minimization using the BFGS (Broyden-Fletcher-Goldfarb-Shanno) algorithm [@Nash1990]. Relies on evaluation of $f$ and its gradient $g(\vec{x}) = \frac{\partial f(\vec{x})}{\partial \vec{x}}$ but not the Hessian. This function directly calls [vmmin][vmmin-src], the C function that gets invoked when using the [optim][Roptim-src] R function with `method = "BFGS"`.
...
1. Initial value for optimization variable.
2. Function $f$ to minimize.
3. Gradient function $g(\vec{x}) = \frac{\partial f(\vec{x})}{\partial \vec{x}}$.
4. Additional arguments.
...
Maximize the function $f(\vec{x}) = \exp(-\vec{x}^\top \vec{x})$. First use the default numerical gradient, then use an explicitly coded the gradient function $g(\vec{x}) = -2 \vec{x} \exp(-\vec{x}^\top \vec{x})$. A C++ function with Rcpp interface is defined in the file `examples/bfgs.cpp`.
...
evaluation of $f$ and its gradient $g(\vec{x}) = \frac{\partial f(\vec{x})}{\partial \vec{x}}$
...
Maximize the function $f(\vec{x}) = \exp(-\vec{x}^\top \vec{x})$. First use the default numerical gradient, then use explicitly coded gradient function $g(\vec{x}) = -2 \vec{x} \exp(-\vec{x}^\top \vec{x})$. A C++ function with Rcpp interface is defined in the file `examples/lbfgsb.cpp`.
...
Minimization using the conjugate gradient algorithm [@FletcherReeves1964; @Nash1990]. Relies on evaluation of $f$ and its gradient $g(\vec{x}) = \frac{\partial f(\vec{x})}{\partial \vec{x}}$ but not the Hessian. This function directly calls [cgmin][cgmin-src], the C function that gets invoked when using the [optim][Roptim-src] R function with `method = "CG"`.
...
Maximize the function $f(\vec{x}) = \exp(-\vec{x}^\top \vec{x})$. First use the default numerical gradient, then use an explicitly coded the gradient function $g(\vec{x}) = -2 \vec{x} \exp(-\vec{x}^\top \vec{x})$. A C++ function with Rcpp interface is defined in the file `examples/cg.cpp`.
...
The gradient $g(\vec{x}) = \frac{\partial f(\vec{x})}{\partial \vec{x}}$ and Hessian $h(\vec{x}) = \frac{\partial^2 f(\vec{x})}{\partial \vec{x} \partial \vec{x}^\top}$ may be provided explicitly if available. When the gradient and/or Hessian are not specified, numerical approximations are used by `optif9`.
...
Maximize the function $f(\vec{x}) = \exp(-\vec{x}^\top \vec{x})$. The associated gradient and Hessian functions are $g(\vec{x}) = -2 f(\vec{x}) \vec{x}$ and $h(\vec{x}) = (4 \vec{x} \vec{x}^\top - 2 \vec{I}) f(\vec{x})$, respectively. Consider three calls: first with a numerical gradient and Hessian, then with explicitly coded gradient, and finally with both explicitly coded gradient and Hessian. A C++ function with Rcpp interface is defined in the file `examples/nlm.cpp`.
...
What is `csc`? `csr`? `coo`?
Consisting of the row index $i$, column index $j$, and value $v_{ij}$ of each element.
Where are these formats from? How can our containers be used with `Matrix`, `Armadillo` and other libraries for matrix work?
Mention the converters.
:::

## Apply {#sec-matrix-apply}

::: {.callout-note title="TBD"}
Add material for updated code. We need some examples to clarify usage of the template functions.
:::


Apply a function to the elements, rows, or columns of an $m \times n$ matrix.
Suppose $\vec{X} \in \mathbb{R}^{m \times n}$ is a matrix with rows
$\vec{x}_{1 \ast}, \ldots, \vec{x}_{m \ast}$ and columns
$\vec{x}_{\ast 1}, \ldots, \vec{x}_{\ast n}$.

The function `mat_apply` is an elementwise application of
$f : \mathbb{R} \rightarrow \mathbb{R}$ which computes
<!-- -->
$$
\text{\texttt{mat\_apply}}(\vec{X}, f)
= \begin{bmatrix}
...
<!-- -->
$$
\text{\texttt{row\_apply}}(\vec{X}, g)
= \big( g(\vec{x}_{1 \ast}), \ldots, g(\vec{x}_{m \ast}) \big).
$$
<!-- -->
...
<!-- -->
$$
\text{\texttt{col\_apply}}(\vec{X}, h)
= \big( h(\vec{x}_{\ast 1}), \ldots, h(\vec{x}_{\ast n}) \big).
$$
<!-- -->
...
The functions  `mat_apply`, `row_apply`, and `col_apply` correspond to elementwise, rowwise, and columnwise apply. Primary location of the source code is the file [inst/include/apply.h][apply-h]. A number of template arguments are defined so that the functions can operate on many types of elements. Template arguments will be implicitly determined in typical usage; see the examples in the following subsection.

[apply-h]: https://github.com/andrewraim/fntl/blob/main/inst/include/apply.h

The following operate on Rcpp matrices and vectors. The result is either an STL vector or a `mat` container (**TBD:** link to information on `mat`). Note that the type of the result need not be the same as elements of `X`.
...
mat<T> mat_apply(
	const Rcpp::Matrix<RTYPE>& X,                           // <1>
	const std::function<T(S)>& f                            // <3>
)
```

1. An Rcpp matrix object.
2. Function $f$ that takes an Rcpp vector `x` and returns an element of type `T`. Type `T` need not the the same as the elements of `x`.
3. Function $f$ that takes elements of type `S` and returns elements of type `T`. Type `S` should be compatible with the elements of `X` but `T` need not be.

**TBD: Explain templates.** Template argument `RTYPE` represents the Rcpp element type (@sec-rpp-templates) and `T` represents a native C++ type which is compatible with `RTYPE`. The domain and range of function `f` should match the class of `X` and type of apply operation. Template arguments are not explicitly specified when calling the functions; instead, they are inferred by the arguments to the function. See the examples in the following subsection for an illustration.

The following operate on `mat` matrix containers (**TBD** link to section) and STL vectors. These can contain elements of an arbitrary type; therefore, they cannot necessarily be passed back and forth to R.

```{cpp}
template <typename S, typename T>
std::vector<T> row_apply(
	const mat<S>& X,                                        // <1>
	const std::function<T(const std::vector<S>&)>& f        // <2>
)
...
mat<T> mat_apply(
	const mat<S>& X,                                        // <1>
	const std::function<T(const S&)>& f                     // <2>
)
```
1. An `mat` matrix object that contains elements of type `T`.
2. Function $f$ that takes a vector `x` of type `S` elements and returns an element of type `T`.
3. Function $f$ that takes an element `x` of type `S` elements and returns an element of type `T`.

The following operate on sparse matrices. Matrices arguments are represented as `csc_mat` sparse matrix containers (**TBD** link to section). Rows and columns are represented by a pair of STL vectors: one for the values of nonmissing elements and another for their indices. As with `mat`, these can contain elements of an arbitrary type, so cannot necessarily be returned to R directly.

```{cpp}
template <typename S, typename T>
std::vector<T> row_apply(
	const csc_mat<S>& X,                                    // <1>
	const std::function<T(const std::vector<S>&,            // <2>
...
csc_mat<T> mat_apply(
	const csc_mat<S>& X,                                    // <1>
	const std::function<T(const S&)>& f                     // <3>
)
```
1. A `csc_mat` matrix object that contains elements of type `T`.
2. Function $f$ of two arguments, `x` and `rows`, which sparsely represent a column of `X`. The elements of the argument are of type `S` and the result is of type `T`, which need not be the same. The argument `x` contains the values of the non-missing elements of the column and `rows` contains the corresponding (zero-based) row indices.
3. Function $f$ that takes an element `x` of type `S` elements and returns an element of type `T`.

Elements missing from `csc_mat` `X` are skipped in `mat_apply` so that they are also missing in the result.

```{cpp}
template <typename T>
std::vector<T> coord_apply(
    unsigned int n,                                                        // <1>
    const std::function<T(unsigned int)>& f)                               // <4>

template <typename T>
mat<T> coord_apply(
    unsigned int m,                                                        // <2>
    unsigned int n,                                                        // <3>
    const std::function<T(unsigned int, unsigned int)>& f)                 // <5>

template <typename T>
csc_mat<T> coord_apply_sp(
    unsigned int m,                                                        // <2>
    unsigned int n,                                                        // <3>
    const std::function<std::pair<T,bool>(unsigned int, unsigned int)>& f) // <6>
```
1. Length of sequence to iterate for single index coordinates; i.e., `0, 1, ..., n`.
2. Length of sequence to iterate for row indices; i.e., `0, 1, ..., m`.
3. Length of sequence to iterate for column indices; i.e., `0, 1, ..., n`.
4. Function whose argument is a single index and returns a value of type `T`.
5. Function of two arguments: a row index `i` and a column index `j`. Returns a value of type `T`.
6. Function of two arguments: a row index `i` and a column index `j`. Returns a pair of elements: the first of type `T` is the return value and the second is a `bool` to indicate whether the result should be included in the result; otherwise it is omitted from the sparse representation.

**TBD** Maybe we don't need all these apply functions. I think we could keep just the `coord_apply` ones, but maybe also keep the Rcpp matrix ones since we had them before? But... the extra ones are nice because they gather up rows or columns into a vector for the caller...

**TBD** Consider renaming `coord_apply` and dropping `coord`.

**TBD** For STL vectors, there is already `for_each` and `transform`. We can mention them. Ours may be convenient because they reqire iterators.

**TBD** The `coord_apply` functions are equivalent to calling `outer(x, y, f)` with `x = c(0, ..., m-1)` and `y = c(0, ..., n-1)`. The `coord_apply` functions are a bit more convenient to use.

### Example: Apply for Rcpp Matrices {.unnumbered .unlisted}
...
TBD: Here is a snippet demonstrating `mat_apply` with `X` as a `NumericMatrix` and `f` as a function that transforms a `double` to another `double` by squaring it.

TBD: The following snippet shows a `row_apply` and `col_apply` which compute row and column sums of an integer matrix. Here the argument of `f` is a reference to an Rcpp `IntegerVector` and the return value is an `int`.

TBD: The following snippet operates on a matrix of strings. We compute the elementwise length of each string.
...
words = character(12)
for (i in 1:12) {
	words[i] = rpois(1, lambda = 10) |>
		sample(LETTERS, size = _, replace = F) |>
		paste(collapse = "")
}
Z = matrix(words, 4, 3)
apply3_ex(Z)
...
### Example: Apply for Coordinates {.unnumbered .unlisted}

```{.cpp include="examples/apply-coord.cpp"}
```

**TBD:** This demonstrates apply functions for coordinates.

Call the function from R.

```{r}
Rcpp::sourceCpp("examples/apply-coord.cpp")
x = rnorm(24)
X = matrix(x, 4, 6)
coord_apply1_ex(as.vector(X))
coord_apply2_ex(X)
sp = coord_apply_sp_ex(X)
Matrix::sparseMatrix(sp$i, p = sp$p, x = sp$x, dims = c(sp$m, sp$n))
```

## Outer

Construct a matrix from a function of two arguments. Or use it in a matrix multiplication without explicitly constructing the matrix.


::: {.callout-note title="TBD"}
Should $\vec{a}$ be of type $\mathbb{E}$ or just a vector of scalars? Until now, we were only thinking about numerical matrices.

Do we need this notation for when our sets are rows of matrices? (That is one of the main use cases).
<!-- -->
$\vec{X} \in \mathbb{R}^{n \times d}$ is a matrix with rows $\vec{x}_1, \ldots, \vec{x}_n$.
$\vec{X} \in \mathbb{R}^{m \times d_1}$ is a matrix with rows $\vec{x}_1, \ldots, \vec{x}_m$, $\vec{Y} \in \mathbb{R}^{n \times d_2}$ is a matrix with rows $\vec{y}_1, \ldots, \vec{y}_n$,
<!-- -->
:::

Suppose $f(x, x') : \mathbb{X} \times \mathbb{X} \rightarrow \mathbb{E}$ where $x, x' \in X$ for $X = \{ x_1, \ldots, x_n \} \in \mathbb{X}^n$. We are primarily interested in cases where $\mathbb{E}$ is the real numbers or integers, but other types such as strings may be used. The `outer` operation computes the $n \times n$ symmetric matrix
<!-- -->
$$
\text{\texttt outer}(X, f) =
\begin{bmatrix}
...
$$
<!-- -->
Furthermore, let $\vec{a} = [a_1 \cdots a_n]^\top$ be a vector in $\mathbb{R}^{n \times 1}$ and suppose the operation $\sum_{j=1}^n a_j u_j$ is defined for $u_j \in \mathbb{E}$ and $\sum_{j=1}^n a_j u_j \in E$. The `outer_matvec` operation computes the vector
<!-- -->
$$
\text{\texttt outer\_matvec}(X, f, \vec{a}) =
\begin{bmatrix}
f(x_1, x_1) & \cdots & f(x_1, x_n) \\
\vdots & \ddots & \vdots \\
f(x_n, x_1) & \cdots & f(x_n, x_n) \\
...
=
\begin{bmatrix}
\sum_{j=1}^n a_j f(x_1, x_j) \\
\vdots \\
\sum_{j=1}^n a_j f(x_n, x_j) \\
\end{bmatrix}.
$$
<!-- -->
Now suppose $f(x,y) : \mathbb{X} \times \mathbb{Y} \rightarrow \mathbb{E}$
where $x \in X$ and $y \in Y$ for $X = \{ x_1, \ldots, x_m \} \in \mathbb{X}^m$ and $Y = \{ y_1, \ldots, y_n \} \in \mathbb{Y}^n$. The `outer` operation computes the $m \times n$ matrix
<!-- -->
$$
...
<!-- -->
$$
\text{\texttt outer\_matvec}(X, Y, f, \vec{a}) =
\begin{bmatrix}
f(x_1, y_1) & \cdots & f(x_1, y_n) \\
\vdots & \ddots & \vdots \\
f(x_m, y_1) & \cdots & f(x_m, y_n) \\
...
=
\begin{bmatrix}
\sum_{j=1}^n a_j f(x_1, y_j) \\
\vdots \\
\sum_{j=1}^n a_j f(x_m, y_j) \\
\end{bmatrix}.
$$
<!-- -->
The `outer` functions above are similar to the `outer` function in R.

An important special case for `outer` is where $x_i$ and $y_j$ are rows from respective matrices $\vec{X}$ and $\vec{Y}$.

The function $\text{\texttt outer\_sp}(X, f, g)$ is analogous to the previously introduced $\text{\texttt outer}(X, f)$, but returns a `csc` sparse representation of the matrix. The additional argument $g(x_i, x_j) : \mathbb{X} \times \mathbb{X} \rightarrow \{ 0, 1 \}$ is an indicator function that determines whether the $(i, j)$th element is included in the result. This is especially useful when computing large matrices with relatively few nontrivial elements. Omitted elements are conventionally taken to represent zeros, but may be reserved for other values such as `Inf`, `-Inf`, or numbers outside of a given interval. Similarly, the function $\text{\texttt outer\_sp}(X, Y, f, g)$ is analogous to $\text{\texttt outer}(X, Y, f)$ with argument $g(x_i, y_j) : \mathbb{X} \times \mathbb{Y} \rightarrow \{ 0, 1 \}$ that determines which elements are included in the result.
...
5. Indicator function $g(\vec{x}, \vec{y})$ which determines whether to include each triplet in the output.

### Result {.unnumbered .unlisted}

**TBD:** Define the matrices up front and replace new classes for this one.

```{cpp}
struct csc_dmat
{
	std::vector<unsigned int> i;  // <1>
	std::vector<unsigned int> j;  // <2>
...
1. Vector of integers which represent row indices for triplets.
2. Vector of integers which represent column indices for triplets.
3. Vector of numerical values of elements for triplets.
4. Number of rows in the matrix.
5. Number of columns in the matrix.
6. Conversion operator to `Rcpp::List`.

Indices in vectors `i` and `j` are zero-based in C++ so that their elements are in $\{0, \ldots, m-1 \}$ and $\{0, \ldots, n-1 \}$, respectively. If the struct is wrapped to an `Rcpp::List`, those indices are incremented by 1 to be one-based.

### Example 1 {.unnumbered .unlisted}

Compute the distance $(\vec{x},\vec{x}')$ between pairs of rows of $\vec{X}$, then compute the distance $f(\vec{x},\vec{y})$ between each pair of rows from $\vec{X}$ and $\vec{Y}$. Then multiply the respective matrices by $\vec{a} = [1, \ldots, 1]$. Lastly, construct triplet representations of the two distance matricesn where elements with absolute distance $f(\vec{x},\vec{y}) \geq 2$ are omitted. A C++ function with Rcpp interface is defined in the file `examples/outer.cpp`.
...
Construct sparse matrices from each of the sparse results using the function `sparseMatrix` from the `Matrix` package [@Matrix]. The first matrix is to be interpreted as the lower triangular block of a symmetric matrix.
...
sp1 = out[[5]]
sp2 = out[[6]]
sparseMatrix(sp1$i, p = sp1$p, x = sp1$x, dims = c(sp1$m, sp1$n), symmetric = T)
sparseMatrix(sp2$i, p = sp2$p, x = sp2$x, dims = c(sp2$m, sp2$n), symmetric = F)
```

### Example 2 {.unnumbered .unlisted}

Recompute one of the sparse matrices from the previous example, but this time convert to an Armadillo sparse matrix in C++; this object is returned as an R sparse matrix via `RcppArmadillo`. A C++ function with Rcpp interface is defined in the file `examples/outer-sp.cpp`.

```{.cpp include="examples/outer-sp.cpp"}
```

Call the function from R.

```{r}
Rcpp::sourceCpp("examples/outer-sp.cpp")
outer_sp_ex(X)
```

## Matrix-Free Linear Solve

Solve a linear system $\vec{A} \vec{x} = \vec{b}$ for symmetric positive definite $\vec{A} \in \mathbb{R}^{n \times n}$ using the conjugate gradient method [e.g., @NocedalWright2006, Chapter 5], where the operation $\vec{A} \vec{x}$ is specified through a function $\ell(\vec{x}) = \vec{A} \vec{x}$. This can be used to avoid explicit storage of large matrices when $\vec{A}$ is sparse. The solution $\vec{x}^*$ is found by minimizing the quadratic function
<!-- -->
$$
f(\vec{x}) = \frac{1}{2} \vec{x}^\top \ell(\vec{x}) - \vec{b}^\top \vec{x},
$$
<!-- -->
so that $f'(\vec{x}^*) = 0 \iff \vec{A} \vec{x}^* = \vec{b}$. Minimization is carried out using the conjugate gradient method in @sec-cg.
...
2. The vector $\vec{b} \in \mathbb{R}^{n}$.
3. Initial value for $\vec{x}$.
...
Solve the equation $\vec{A} \vec{x} = \vec{b}$ with $n \times n$ matrix $\vec{A}$ having value $2$ on the main diagonal and value 1 on the upper and lower diagonals; let $\vec{b} = [1, \ldots, 1]$. The initial value for the solution is taken to be the default $\vec{x} = \vec{0}$. A C++ function with Rcpp interface is defined in the file `examples/solve-cg.cpp`.
...
::: {.callout-note title="TBD"}
Add material for sparse version. For this one, we skip the missing elements and do not attempt to interpret their values.
:::

Identify the indices of a matrix which satisfy a given indicator function. Specifically, let $\vec{X} \in \mathbb{S}^{m \times n}$ be a matrix whose elements are in the domain $\mathbb{S}$ which may be doubles, integers, or another Rcpp `Matrix` type. Let $f : \mathbb{S} \rightarrow \{0, 1\}$ be an indicator function and suppose $k$ of the $mn$ elements in $\vec{X}$ satisfy the indicator. The `which` operation produces a $k \times 2$ matrix
<!-- -->
$$
\text{which}(\vec{X}, f) =
\begin{bmatrix}
...
$$
<!-- -->
where each pair $(i_\ell, j_\ell)$ are coordinates of an element in $\vec{X}$ that satisfies $f$. Indices $i_\ell$ and $j_\ell$ represent the row and column index, respectively. Indices are *zero-based* by default as they are primarily intended to be used in C++ code.
...
2. A CDF whose arguments are: (1) the argument `x` for which to evaluate
   the CDF, (2) a boolean `lower` that determines whether the lower-tail
...
3. A quantile function whose arguments are: (1) the argument `p` for which to
   ...
   `log` that determines whether `p` is assumed to be on the log-scale (if
   `true`).
