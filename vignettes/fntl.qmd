...
Density, quantiles, cumulative distribution function (CDF), and variate
generation for a univariate distribution truncated to the interval $(a , b]$,
given density $f$, CDF $F$, and quantile function $F^{-}$ of an untruncated
distribution. Originally adapted from the `rtruncated` function in the
`LearnBayes` package [@LearnBayes].
...
on the CDF are considered using both lower and upper orientations to aid with
