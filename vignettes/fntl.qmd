...
::: {.callout-note title="TBD"}
Add material for sparse version. For `mat_apply`, we skip the missing elements and do not attempt to process their values. For `row_apply` and `col_apply`, we take a vector with the non-missing elements and another vector with their respective row or column indices.
:::
