...
Compute the Jacobian of $f(\vec{x}) = [f_1(\vec{x}), \ldots, f_5(\vec{x})]$, $f_i(\vec{x}) = \sum_{j=1}^i \sin(x_j)$, at the point $\vec{x} = (1,2,3,4,5)$. To obtain the resulting value as an $m \times n$ matrix, we apply the List conversion operator to the result. A C++ function with Rcpp interface is defined in the file `examples/jacobian.cpp`.
...
obtain the resulting value as an $n \times n$ matrix, we apply the List conversion operator to the result. A C++ function with Rcpp interface is defined in the file `examples/hessian.cpp`.
