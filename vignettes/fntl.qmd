...
Solve a linear system $\vec{A} \vec{x} = \vec{b}$ for symmetric positive definite $\vec{A} \in \mathbb{R}^{n \times n}$ [@NocedalWright2006]. Here the operation $\vec{A} \vec{x}$ is specified through a function $\ell(\vec{x}) = \vec{A} \vec{x}$. This can be used to avoid explicit storage of large matrices when $A$ is sparse. The solution $\vec{x}^*$ is found by minimizing the quadratic function
<!-- -->
$$
