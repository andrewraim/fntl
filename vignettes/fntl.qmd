...
## Optional Arguments {.unlisted .unnumbered}
...
## Status Codes {.unlisted .unnumbered}
...
The functions $\text{\texttt outer\_triplet}(\vec{X}, f, g)$ and $\text{\texttt outer\_triplet}(\vec{X}, \vec{Y}, f, g)$ are analogous to the previously introduced $\text{\texttt outer}$ functions, but instead return a triplet representation consisting of the row index $i$, column index $j$, and value $v_{ij}$ of each element. The additional argument $g(\vec{x}_i, \vec{x}_i) : \mathbb{R}^d \times \mathbb{R}^d \rightarrow \{ 0, 1 \}$ is an indicator function for whether the corresponding $(i, j, v_{ij})$ triplet is included in the result. This can be used to construct sparse matrices and is especially useful when working with large matrices with relatively few nontrivial elements.
...
6. If `true`, row and column indices are one-based; otherwise they are zero-based.
7. Conversion operator to `Rcpp::List`.
