...
elementwise, rowwise, and columnwise apply. Primary location of the source code is the file [inst/include/apply.h][apply-h]. A number of template arguments are defined so that the functions can operate on many types of elements. Template arguments will be implicitly determined in typical usage; see the examples in the following subsection.

[apply-h]: https://github.com/andrewraim/fntl/blob/main/inst/include/apply.h

The following operate on Rcpp matrices and vectors. The result is either an STL vector or a `mat` container (**TBD:** link to information on `mat`).
...
The following operate on `mat` matrix containers (**TBD** link to section). These can contain elements of an arbitrary type that cannot necessarily be passed back and forth to R.
...
The following operate on `csc_mat` sparse matrix containers (**TBD** link to section). As with `mat`, these can contain elements of an arbitrary type that cannot necessarily be passed back and forth to R.
...
**TBD:** Do we want another variant (or flag) that lets us process missing elements too? How should we indicate that we want the result to have a missing value? Maybe we should have a second function `g` for this? Or we can take a second function `g` that processes missing values.

**TBD:** Apply functions may want access to the coordinate in the matrix. And maybe access to other elements in the matrix? Do we want to have another batch of apply functions, or extend the current ones to support this? Does "extended apply" go by another name in R or other platforms? I'm thinking we just stick to R-style apply...

**TBD** Another function we might want is to map/apply a function to an Rcpp List. 

**TBD** For STL vectors, there is already `for_each` and `transform`. We can mention that.
...
Construct a matrix from a function of two arguments. Or carry out a matrix multiplication without explicitly constructing the matrix.
